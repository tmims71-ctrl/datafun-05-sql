"""tmims_duckdb_civic_event.py - Project script (civic_event).
...
def main() -> None:
    """Run the pipeline."""
    log_header(LOG, "P05 Pipeline (DuckDB) - Civic Event")
    ...
    try:
        ...
        # ----------------------------------------------------
        run_sql_script(con, SQL_DIR / "tmims_civic_event_clean.sql")

        # ----------------------------------------------------
        # STEP 2: BOOTSTRAP (create tables, load CSV data)
        # ----------------------------------------------------
        run_sql_script(con, SQL_DIR / "tmims_civic_event_bootstrap.sql")

        # ----------------------------------------------------
        # STEP 3: RUN BASIC QUERIES
        # ----------------------------------------------------
        run_sql_query(con, SQL_DIR / "tmims_civic_event_query_event_count.sql")
        run_sql_query(con, SQL_DIR / "tmims_civic_event_query_attendance_count.sql")
        run_sql_query(con, SQL_DIR / "tmims_civic_event_query_attendance_aggregate.sql")
        run_sql_query(con, SQL_DIR / "tmims_civic_event_query_attendance_by_type.sql")
